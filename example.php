<?php
/**
 * Contoh Script Bridge API Netshort via StreamAPI.web.id
 * ----------------------------------------------------
...
// Fungsi bantuan buat manggil endpoint Netshort di sistem Bridge
function callNetshortAPI($urlTujuan, $token) {
...
// UJI COBA KODE
// -----------------------------------------------------------------
$baseUrl = "https://streamapi.web.id/p/netshort/api/v1";

// Contoh 1: Fitur Cari Film Netshort
echo "\n=====================[ 1. Pencarian Netshort ]==================\n";
$kataKunci = urlencode("cinta");
$halaman = 1;
$urlCari = "{$baseUrl}/search/{$kataKunci}/{$halaman}?lang=in";

$hasilCari = callNetshortAPI($urlCari, $API_TOKEN);
if ($hasilCari) {
...
$hasilDetail = callNetshortAPI($urlDetail, $API_TOKEN);
if ($hasilDetail) {
...
$hasilNonton = callNetshortAPI($urlNonton, $API_TOKEN);
if ($hasilNonton) {
...
echo "▶▶▶ https://streamapi.web.id/docs/netshort ◀◀◀\n\n";
